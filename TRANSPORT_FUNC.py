...
def transport(t1,shift,l, rho):
    ...
    #INTERPOLATION
    f = spint.interp1d(r_trans, f_tran*rho[:ind_rc1], fill_value = 'extrapolate')
    r_interp = r[ind_low_trans:]
    rho_trans_interp = f(r_interp)

    dens_final = np.zeros(l)
    dens_final[:ind_rc1] += f_rem*rho[:ind_rc1]
    dens_final[ind_low_trans:] +=  rho_trans_interp 
    return dens_final, r
