...
#load_dir = direc + '/pot_stop/analysed_data/'
# = '/home/kyle/Pictures/'
...
for a in range(0,len(sig)):
    ...
    m = format(sig[a], '.2f')
    for p in range(1, lp, p_inc):
        file = np.loadtxt(load_dir + 'elec_dens_peak'+str(int(pel_pot[p]))+'sig_' + m +'.txt')
        x = file[:,0]
        y = file[:,1]
        x = x[:-1]
        y = y[:-1]
...
ax.view_init(55,150)
#plt.savefig(load_dir + 'gauss_pot_elec_bins_wpot.png', format = 'png', dpi = 1400)
plt.show()        
