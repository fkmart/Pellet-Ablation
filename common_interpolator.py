...
def common_interp(r, A1, r1): #make sure r1 and r2 are in the common grid with pellet centre at zero of the coord system 
    ...
    ind_interp_low = next(p[0] for p in enumerate(r) if p[1] >= r1[0])
    ind_interp_up = next(p[0] for p in enumerate(r) if p[1] >= r1[-1]) 

    g = spint.interp1d(r1, A1, kind = 'cubic')

    A1_new = g(r[ind_interp_low:ind_interp_up])
