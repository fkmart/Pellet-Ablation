...
def common_interp(r, A1, r1): #make sure r1 and r2 are in the common grid with pellet centre at zero of the coord system 
    ...
    ind_interp_low = next(p[0] for p in enumerate(r) if p[1] >= r1[0]) # lowest common point
    ind_interp_up = next(p[0] for p in enumerate(r) if p[1] >= r1[-1]) # greatest common point
