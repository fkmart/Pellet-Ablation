...
def elec_mover(t_ind, r, eden, rden):
    ...
    dens_diff = dens1[:] - dens2[indl1-indl2:indu1-indu2] 

    #Now calculate the flowspeeds and fractional changes
    frac_change = np.zeros(indu1 - indl1)
    const = dens1[-1]*rcd[t_ind]
    flowspeed = np.zeros(indu1-indl1)
    for i in range(indl1,indu1-indl1):
        frac_change[i - indl1] = dens_diff[i- indl1]/dens1[i - indl1]
        flowspeed[i - indl1] = const/dens1[i - indl1] 

    #Now interpolate the densities to all internal gridpoints 
    g = spit.interp1d(rden + rp1,eden, kind = 'linear')
    ...
    eden_grid_rem = (1.0 - frac_change[ind_low:-ind_up])*eden_grid[:]

    eden_grid_trans = eden_grid[:]*frac_change[ind_low: -ind_up]

    r_trans = r[ind_interp_low:ind_interp_up] + flowspeed[ind_low:-ind_up]*dt*shift
    ...
    f = spit.interp1d(r_trans, eden_grid_trans, kind = 'linear')
