...
if style == 'once':
    ...
    #BE ON THE LOOKOUT FOR CODE FOR A GAUSSIAN TEST POTENTIAL 
    i = many_start
    A0 = 500.0
    A0_norm = A0/(RME*M_fac)
    pot = np.zeros(lr)
    count = 0
    adv_pot = np.zeros(lr)
    while rp[i] > rp_crit:
        ...
        r_internal = r[low:up]
        if (i == many_start):
            cent = int(0.5*(up+low)) - low
            mid = r_internal[cent]
            k = sig_calc(A0_norm,mid,r_internal[-1], 1.0/(M_fac*RME))
        else:
            mid = gcf.gcf(faux_density[:,1], faux_density[:,0])
            k = sig_calc(A0_norm,mid,r_internal[-1],0.01/(M_fac*RME))
        pot_int = gauss_test_pot.gauss_func(A0_norm,k, mid ,r_internal)
        pot[low:up] = adv_pot[low:up] - pot_int[:]

        #make potential array for visualisation later
        pot_arr = np.copy(pot[low:up]*RME*M_fac)
        pot_arr = np.append(pot_arr, r_internal)
        pot_arr = np.reshape(pot_arr, (int(len(pot_arr)*0.5),2), 'F')
        ...
        #Now to advect the potential with the flow of neutrals + electrons as an additional test
        adv_pot, ind1, ind2 = e_trans.elec_mover(i,r,pot,r_internal,shift) 
        adv_pot_arr = np.copy(adv_pot[ind1:ind2]*RME*M_fac)
        adv_pot_arr = np.append(adv_pot_arr, r[ind1:ind2])
        adv_pot_arr = np.reshape(adv_pot_arr, (int(len(adv_pot_arr)*0.5),2), 'F')
        ...
        np.savetxt(os.path.join(savedir_an, 'density_pot_test_t' +str(i) +'.txt'), faux_density)
        np.savetxt(os.path.join(savedir_an, 'real_density_pot_test_t'+str(i) +'.txt'), real_density)
        np.savetxt(os.path.join(savedir_an,'potential_update_test_t'+str(i) + '.txt'),pot_arr)
        np.savetxt(os.path.join(savedir_an, 'advected_potential_t' + str(i) + '.txt'), adv_pot_arr)
        i += shift # increase time counter to save incremental data
        count +=1
        if (count == 4):
            print("That's enough peaks to make the point. Time to stop")
            sys.exit()
        else:
            pass
