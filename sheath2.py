...
phi, x = RK4.RK4_t(sheath_final,x0,h, phi_1, phi_s)
x *= ld*K
phi *= phi_0
...
phi_s_real = phi_s*phi_0
phi_1_real =phi_0*phi_1
phi_real = np.logspace(np.log10(phi_1_real), np.log10(phi_s_real), l)
x_real = RK4.RK4_y(sheath_real, x0, phi_real)

x_diff = x_real - x

ax.plot(x_real*(2.0*e*n0/eps0)**(-0.5), phi_real)
ax.set_yscale('log')
ax.set_xscale('log')
plt.grid()
plt.show()
