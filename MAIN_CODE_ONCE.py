...
for i in range(150, 200, 100):
    ...
    #r_grid = np.linspace(rp[i], rc[i], num = 1024, endpoint = 'true')
    r_grid = np.arange(rp[i], rc[i], step = 1e-3)
    ...
    np.savetxt(os.path.join(save_an_t, 'norm_density.txt'), np.asarray([r_grid,norm_density]))


"Saving compilation of Bethe calculations with varying potentials"
#np.savetxt(os.path.join(save_an_t, 'retarded_flux.txt'), flux_arr)    
#np.savetxt(os.path.join(save_an_t, 'retarded_ener_flux.txt'), ener_flux_arr)
#np.savetxt(os.path.join(save_an_t, 'pellet_lifetime.txt'), lifetime_arr)
print('success')